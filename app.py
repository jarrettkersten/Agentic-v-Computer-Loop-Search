...
def run_hybrid_loop(query, branch):
    ...
    SYNTH_SYSTEM = (
        ...
        "Cora PPM frequently reuses the same button label, feature name, or function name across "
        "DIFFERENT pages/UIs (e.g. 'Load From Schedule' exists on both the Resource Allocation page "
        "AND the Project Forecasting page, doing completely different things). If the provided files "
        "contain implementations from MULTIPLE pages for the same feature name, you MUST describe each "
        "page's version SEPARATELY with clear headings (e.g. '### On the Resource Allocation page' vs "
        "'### On the Project Forecasting page'). NEVER merge or conflate them into one explanation. "
        "If only one page's implementation was found, explicitly note that the same feature name may "
        "exist on other pages whose code was not included.\n\n"
...
def run_sharepoint_loop(query: str) -> dict:
    ...
    synthesis = client.messages.create(
        ...
        system=(
            ...
            "DISAMBIGUATION: If the documents describe the same feature name (e.g. a button label) "
            "on DIFFERENT pages or UIs, you MUST describe each page's version SEPARATELY with clear "
            "headings. NEVER merge or conflate them into one explanation.\n\n"
...
def synthesize_all(query: str, agentic_result: dict,
    ...
    resp = client.messages.create(
        ...
        system=(
            ...
            "⚠️ CRITICAL — DISAMBIGUATION (SAME NAME, DIFFERENT UI):\n"
            "Cora PPM frequently reuses the same button label, feature name, or function name across "
            "DIFFERENT pages/UIs. For example, 'Load From Schedule' exists on both the Resource "
            "Allocation page AND the Project Forecasting page, but they do completely different things.\n"
            "  - NEVER assume two pages with the same button/feature name implement the same logic.\n"
            "  - If the source answers describe implementations from DIFFERENT pages, you MUST keep "
            "    them SEPARATE in your synthesis — use clear sub-headings for each page/UI.\n"
            "  - If the source answers only cover ONE page's implementation but the question mentions "
            "    or implies multiple pages, explicitly state which page WAS covered and which was NOT.\n"
            "  - When the question names two UIs with a slash (e.g. 'Resource Allocation / Project "
            "    Forecasting'), treat these as SEPARATE UIs that need separate explanations, NOT as "
